...
stan_sat_model <- stan_model("stan/stan.stan", "sat_model")
...
save(sat_model_fit, file = "stan/sat_model_fit_v0_0_1.RData")
```
...
cor_mat <- fit_tbl |> 
  mutate(Chain = rep(1:4, each  = 1500), Iteration = rep(1:1500, 4)) |>
...
(corr_tbl <- as_tibble(cor_mat) %>%
  ...
  # filter(is.na(cor)) %>% 
  filter(v1 < v2) %>%
  filter(across(c(v1,v2), \(v)str_detect(v, "raw\\[|sigma|lp_")), !is.na(cor)) %>%
  arrange(-cor))
...
np <- nuts_params(sat_model_fit) |> as_tibble()
np
