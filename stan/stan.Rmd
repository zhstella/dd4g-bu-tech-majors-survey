...
pred_course <- function(ccd, fit_tbl_long){
  fit_tbl_long_response <- fit_tbl_long |> filter(type == "response")
  ...
  ccd_id <- ccd |> 
    ...
    mutate(gid = cur_group_id(), .by = -c(response, count_id, n)) 
  ccd_id |>
    pivot_longer(-c(count_id, n, gid), names_to = "type", values_to = "name") |> 
    left_join(fit_tbl_long, join_by(type, name), relationship = "many-to-many") |> 
    select(-stan_param) |> 
    group_by(mc_id, gid, count_id) |> 
    mutate(value = if_else(type == "response", value, -value)) |> 
    summarize(value = sum(value, na.rm = TRUE), .groups = "drop_last") |> 
    left_join(ccd_id, by = join_by(gid, count_id)) |> 
    mutate(response = ordered(response, levels = satisfaction_level)) |> 
    arrange(gid, mc_id, response) |> 
...
ccd <- course_count_df |> 
  ...
  filter(
    course == "CAS CS111" | 
      (dept == "CAS CS" & course == "NA" & level == "Lower")
  )
...
ccd_pred |> 
  ...
  stat_summary(aes(y = pobs), geom = "col", fun.y = mean, data = \(d) filter(d, !is.na(pobs))) +
  stat_summary(aes(y = p), fun.data = mean_sdl)
```
